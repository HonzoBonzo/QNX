...
#include "msg_def.h"  //layout of msg's should always defined by a struct, here's it's definition
...
main(void) {
	...
	while(1) {
		...
		txt = msg.string_to_cksum;
//		strcpy(txt, msg.string_to_cksum);
		printf("\notrzymalem od klienta wiadmosc: \n%s\n",txt);
